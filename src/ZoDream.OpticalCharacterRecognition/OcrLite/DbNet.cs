...
namespace ZoDream.OpticalCharacterRecognition.OcrLite
{
    class DbNet
    {
        ...
        private static List<TextBox> GetTextBoxes(DisposableNamedOnnxValue[] outputTensor, int rows, int cols, ScaleParam s, float boxScoreThresh, float boxThresh, float unClipRatio)
        {
            ...
            {
                float minEdgeSize = 0;
                var box = GetMiniBox(contours[i], out minEdgeSize);
                ...
                var newBox = Unclip(box, unClipRatio);
                if (newBox == null)
                {
                    continue;
                }

                var minBox = GetMiniBox(newBox, out minEdgeSize);
        ...
        private static List<PointF> GetMiniBox(VectorOfPoint contours, out float minEdgeSize)
        {
            ...
            var thePoints = new List<PointF>(points);
